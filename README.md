...
The default model for transcription is `gpt-4o-mini-transcribe`, which produced the best results in testing. Do not set `OPENAI_TRANSCRIBE_MODEL` to `gpt-4o-transcribe-diarize`. It is slower and it rewrites wording. For long audio, `whisper-1` is the least likely to reject the file for length, and will still return one block of text before the labeling step.
